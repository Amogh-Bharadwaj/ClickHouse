...
4.  Settings applied to the entire server by default or from the [default profile](/reference/settings/server-settings/settings/default#default_profile).
...
<Note>
A row policy is not a boundary on engines which merge several rows into one (`SummingMergeTree`, `AggregatingMergeTree`, `CoalescingMergeTree`, `GraphiteMergeTree`), because the merged row carries the values of the hidden rows as well, see [engines which merge rows](/reference/statements/create/row-policy#blending-table-engines).
</Note>
...
- Setup a directory for configuration storage.
    ...
    ClickHouse stores access entity configurations in the folder set in the [access_control_path](/reference/settings/server-settings/settings/access-control#access_control_path) server configuration parameter.
